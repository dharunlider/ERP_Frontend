...
import Newstaff from '../HrmStaff/EditStaff';
import EditStaff from '../HrmStaff/Newstaff';
...
const Staff = () => {
  ...
  const fetchStaffData = useCallback(async (reset = false) => {
    ...
    try {
      ...
      const response = await axios.get('/staff/get-inactive-staffs', { params });
  ...
  useEffect(() => {
    fetchStaffData(true);
  }, [refreshKey, debouncedSearchTerm, fetchStaffData]);
  ...
  const handleExportClick = () => {
    downloadFile({
      url: "/staff/export/inactiveOrNull",
      filename: "Inactivestaff.xlsx",
