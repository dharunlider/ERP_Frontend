...
const Newstaff = ({ onAddStaff, setActiveTab }) => {
  ...
  const handleImageUpload = async (event) => {
    ...
    try {
      const cloudinaryUrl = await uploadFileToCloudinary(file, 'profile');
      if (cloudinaryUrl) {
  ...
  const handleAadhaarUpload = async (event) => {
    ...
    const cloudinaryUrl = await uploadFileToCloudinary(file, 'aadhaar');
    if (cloudinaryUrl) {
  ...
  const handlePanCardUpload = async (event) => {
    ...
    const cloudinaryUrl = await uploadFileToCloudinary(file, 'pan');
    if (cloudinaryUrl) {
  ...
  const handlePassportUpload = async (event) => {
    ...
    const cloudinaryUrl = await uploadFileToCloudinary(file, 'passport');
    if (cloudinaryUrl) {
  ...
  //               ✓ Document successfully uploaded to Cloudinary
  //             </Typography>
  //           )}
  //         </Box>
  //       </Box>
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12}>
              ...
              <DocumentUploadField
                ...
                description="Upload Aadhaar card (PDF, JPG, PNG - max 10MB)"
                fileType="aadhaar"
              ...
              <DocumentUploadField
                title="PAN CARD"
                description="Upload PAN card (PDF, JPG, PNG - max 10MB)"
                fileType="pan"
