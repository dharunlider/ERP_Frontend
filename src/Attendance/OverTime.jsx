...
import { Add, Search, Edit, Delete, Cancel, Download } from '@mui/icons-material';
...
const OverTime = () => {
    ...
    const handleExportExcel = async () => {
        try {
            ...
            const response = await axios.get(`/overtime/excel/export?year=${year}&month=${month}`, {
            ...
            link.setAttribute('download', `overtime_report_${formattedDate}.xlsx`);
                    ...
                    {isStaff && (
                        <TabPanel value={state.activeTab} index={0}>
                            <Box display="flex" flexDirection={isMobile ? 'column' : 'row'}
                                ...
                                {/* Export Button for Admin in Pending Tab */}
                                {isAdmin && (
                                    <Button 
                                        variant="contained" 
                                        color="sucprimarycess" 
                            ...
                            <Box sx={{ maxHeight: '60vh', overflow: 'auto' }}>
                                <Table stickyHeader size="small" sx={{ mb: 4, ...centeredTableStyles }}>
                                    <TableHead>
                                        <TableRow>
                                            {['S.NO', 'NAME', 'OT DATE', 'DURATION', 'STATUS', 'ASSIGN NAME', 'REASON', 'OPTIONS'].map((header) => (
                                                    ...
                                                    <TableCell>
                                                        <Box display="flex" justifyContent="center">
                                                            <Tooltip title="Edit">
                                                                <span>
                                                                    <IconButton
                                                                        onClick={() => handleModal(true, true, row)}
                                                                        color="primary"
                    ...
                    {isStaff && (
                        <TabPanel value={state.activeTab} index={1}>
                            <Box display="flex" flexDirection={isMobile ? 'column' : 'row'}
                                ...
                                {/* Export Button for Admin in Processed Tab */}
                                {isAdmin && (
                                    <Button 
                                        variant="contained" 
                                        color="primary" 
                            ...
                            <Box sx={{ maxHeight: '60vh', overflow: 'auto' }}>
                                <Table stickyHeader size="small" sx={centeredTableStyles}>
                                    <TableHead>
                                        <TableRow>
                                            {['S.NO', 'NAME', 'OT DATE', 'DURATION', 'ASSIGN NAME', 'REASON', 'STATUS'].map((header) => (
                                                <TableCell key={header}>{header}</TableCell>
                                            ))}
                                        </TableRow>
                                    </TableHead>
                                    <TableBody>
                                        {filteredProcessed.length > 0 ? (
                                            filteredProcessed.map((row, index) => (
                                                <TableRow key={row.id}>
                                                    <TableCell>{index + 1}</TableCell>
                                                    <TableCell>{row.staffName.toUpperCase()}</TableCell>
                                                    <TableCell>{row.otDate}</TableCell>
                    ...
                    {isAdmin && (
                        <TabPanel value={state.activeTab} index={isStaff ? 2 : 0}>
                            <Box display="flex" flexDirection={isMobile ? 'column' : 'row'}
                                ...
                                {/* Export Button for Admin in All Requests Tab */}
                                <Button 
                                   variant="contained" 
                                    color="primary" 
                                    onClick={handleExportExcel}
